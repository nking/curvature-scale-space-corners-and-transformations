...
public class BaseTwoPointTest extends TestCase {
    ...
    protected float[] xc = null;
    protected float[] yc = null;
    protected float[] xErrors = null;
    protected float[] yErrors = null;

    protected enum CLUSTER_SEPARATION {
    ...
    protected DoubleAxisIndexer createIndexerWithRandomPoints(float xmin,
        ...
        Logger.getLogger(this.getClass().getSimpleName()).info(
            "Creating points: " + nClusters.length + " clusters, "
    ...
    protected DoubleAxisIndexer createIndexerWithRandomPoints(SecureRandom sr, float xmin, float xmax, float ymin, float ymax,
        ...
        for (int i = 0; i < numberOfClusters; i++) {
            nClusters[i] = + minimumNumberOfPointsPerCluster + sr.nextInt(dn);
            count += nClusters[i];
        }
    ...
    protected void createPoints(int numberOfBackgroundPoints, int[] numberOfClusterPoints,
        ...
        for (int i = 0; i < nTotalPoints; i++) {
            ...
            xErrors[i] = x[i] * 0.03f;
            yErrors[i] = (float) (0.5*Math.sqrt(y[i]));
        }
    }
