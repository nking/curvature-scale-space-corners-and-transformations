...
public class StringArrayLiteTest extends TestCase {
    ...
    public void testCreateIdentity() {
        ...
        byte[] bytes0 = StringLite.writeIntegerToBytes(i0, 4);
        ...
        byte[] bytes1 = StringLite.writeIntegerToBytes(i1, 4);
        ...
        byte[] identity = StringLite.createIdentity(i0, i1);
        ...
        bytes0 = StringLite.writeIntegerToBytes(i0, 4);
        assertTrue(bytes0.length == expected0.length);
        for (int i = 0; i < expected0.length; i++) {
            assertTrue(expected0[i] == bytes0[i]);
        }
        ...
        bytes1 = StringLite.writeIntegerToBytes(i1, 4);
        assertTrue(bytes1.length == expected1.length);
        for (int i = 0; i < expected1.length; i++) {
            assertTrue(expected1[i] == bytes1[i]);
        }

        identity = StringLite.createIdentity(i0, i1);
    ...
    public void testHashCode() {
        ...
        byte[] c0 = StringLite.createIdentity(1023456789, 1023456798);
        byte[] c1 = StringLite.createIdentity(987654321, 897654321);
        ...
        c0 = StringLite.createIdentity(1023456789, 10234);
        c1 = StringLite.createIdentity(987654321, 21034);
