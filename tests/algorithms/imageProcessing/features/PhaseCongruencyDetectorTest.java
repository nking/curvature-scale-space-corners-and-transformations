    ...
    public void test4() throws Exception {
        ...
        */
        ...
        for (String fileName : fileNames) {
            ...
            HistogramHolder[] hists = new HistogramHolder[nGroups];
            for (int i = 0; i < nGroups; ++i) {
                TIntList snIndexes = groupIndexes[i];
                // forming the average then intersection w/ average
                ...
                values = new float[snIndexes.size()];
                for (int j = 0; j < values.length; ++j) {
                    int idx = snIndexes.get(j);
                    float intersection = chhist.intersection(sumCHs[i], chs[idx]);
                    values[j] = intersection;
                }
                hists[i] = Histogram.createSimpleHistogram(
                    nBins, values, Errors.populateYErrorsBySqrt(values));            
                hists[i].plotHistogram("intersection group " + i, 
                    "_intersection_" + i);
            }
            // -- for each group, if peak is < a few counts and spread over
            //      a intersection < 0.4, showing large dissim, should look
            //      at whether the members fit with another group and merge if
            //      so, else, mark as dubious or rare.
            //      group 0 in this test shows it is not a real
            //      group, but belongs to others,
            //      but group 3, while being sparse, is a real group.
