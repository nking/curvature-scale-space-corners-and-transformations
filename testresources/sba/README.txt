...
** For a), eucsbademo accepts 3 file names as arguments:
   (1) initial estimates for the camera motion (i.e. pose) parameters, 
       separate line for every camera (i.e. frame):
         7 parameters: 4 element quaternion (scalar first) 3 element translation
            quat: a rotation by angle TH around the unit vector U=(Ux, Uy, Uz) should be specified as
                  cos(TH/2) Ux*sin(TH/2) Uy*sin(TH/2) Uz*sin(TH/2).
   (2) initial estimates for the 3D point parameters and the 3D points image projections
   ...
   (3) the camera intrinsic calibration parameters. 

** For b), eucsbademo accepts 2 file names as arguments, specifically 
   (1) the file defining the inital estimates for the camera intrinsics and motion parameters 
       each line is the 12 parameters for a camera (5 intr: fx in pix, ox in pix, fy/fx, skew,
           quat (scalar term first) 4 elements, translation 3 elements
   (2) and that defining the initial estimates for the 3D point parameters along with the 3D points 
       image projections.  
         same as (2) above
...
** For c), eucsbademo accepts 2 file names as arguments, specifically 
    (1) the file defining the inital estimates for the camera intrinsics, distortions and 
        motion parameters 
        17 params for each camera:  
            5 intr (as above in (1)), 5 distortion params, 4 element quat, 3 element trans., 
    (2) the initial estimates for the 3D point parameters along with the 3D points image projections.
        same as (2) above.
