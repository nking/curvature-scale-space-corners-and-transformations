...
public class ClosedCurveCornerMatcher2<T extends CornerRegion> {
    ...
    private static int tolerance = 2;//4;
    ...
    private void findBestParameters(List<T> c1,
        ...
        */
        ...
        for (int ipt1 = 0; ipt1 < indexes2.length; ++ipt1) {
            if (indexes2[ipt1] == null) {
                continue;
            }
            PairInt c1Pt1 = indexes2[ipt1].stat.getImg1Point();
            PairInt c2Pt1 = indexes2[ipt1].stat.getImg2Point();
            for (int ipt2 = (ipt1 + 1); ipt2 < indexes2.length; ++ipt2) {
                if (indexes2[ipt2] == null) {
                    continue;
                }
...
//TODO: might need to keep the top k and then use feature matching to decide between them        
        ...
        for (TransformationParameters params : combinedParams) {
            ...
            for (int ipt1 = 0; ipt1 < c1.size(); ++ipt1) {
                ...
                // for now, not caring if a point is double matched, just need to know general count
            ...
            if ((nEval2 > maxNEval) || ((nEval2 == maxNEval) && (sumDist < minDist))) {
                maxNEval = nEval2;
                bestParams = params;
                minDist = sumDist;
                minSSD = sumSSD;
