 ...
 * If, at any point, a segment of the path being traversed has a higher cost
 * than another encountered path segment, it abandons the higher-cost path
 * segment and traverses the lower-cost path segment instead. This process
...
public class AStar {
    ...
    public AStar(final PairInt[] points, final List<LinkedList<Integer>> adjacencyList,
        ...
        Arrays.fill(heuristics, Long.MIN_VALUE);
        
        // populate prevNode and destinationNodes when decide between heap nodes or integers
    ...
    private long heuristic(int vIndx) {
        ...
        if (heuristics[vIndx] == Long.MIN_VALUE) {
