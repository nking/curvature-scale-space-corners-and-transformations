...
public class Gates {
    ...
    // TODO: review the use of the variable counter inside and outside this
    //  instance to see whether it should be a class variable or an instance
    //  variable.  assuming the later for now.
    private int counter = 0;
    
    private final float epsilon = 0.000001f;
    
    //TODO: review this class and it's use.
    // counter was a static variable in a method below.
    ...
    */
    ...
    void quantum_swaptheleads(int width, QuantumReg reg) {
        ...
        if (qec[0] > 0) {
            ...
            for (i = 0; i < reg.size; i++) {
                ...
                pat1 = reg.node[i].getState() % QuReg.shiftLeftTruncate(width);

                //calculate right but pattern 
                pat2 = 0;

                for (j = 0; j < width; j++) {
                    pat2 += reg.node[i].getState() & 
                        QuReg.shiftLeftTruncate(width + j);
                }
    ...
    void quantum_gate1(int target, QuantumMatrix m, QuantumReg reg) {
        ...
        if (reg.hashw > 0) {
            ...
            for (i = 0; i < reg.size; i++) {
                ...
                if (QuReg.quantum_get_state(reg.node[i].getState()
                    ^ QuReg.shiftLeftTruncate(target), reg) == -1) {
                    addsize++;
                }
            }
        ...
        limit = (1.0f / QuReg.shiftLeftTruncate(reg.width)) * epsilon;
    ...
    void quantum_gate2(int target1, int target2, QuantumMatrix m,
        ...
        for (i = 0; i < reg.size; i++) {
            if (QuReg.quantum_get_state(reg.node[i].getState() ^ 
                QuReg.shiftLeftTruncate(target1), reg) == -1) {
                addsize++;
            }
            if (QuReg.quantum_get_state(reg.node[i].getState() 
                ^ QuReg.shiftLeftTruncate(target2), reg) == -1) {
                addsize++;
            }
        }
        // allocate memory for the new basis states 
        ...
        limit = (1.0f / QuReg.shiftLeftTruncate(reg.width)) / 1000000.f;
        ...
        for (i = 0; i < reg.size; i++) {
            if (done[i] == 0) {
                j = QuReg.quantum_bitmask(reg.node[i].getState(), 2, bits);
                base[j] = i;
                base[j ^ 1] = QuReg.quantum_get_state(reg.node[i].getState()
                    ^ QuReg.shiftLeftTruncate(target2), reg);
                base[j ^ 2] = QuReg.quantum_get_state(reg.node[i].getState()
                    ^ QuReg.shiftLeftTruncate(target1), reg);
                base[j ^ 3] = QuReg.quantum_get_state(reg.node[i].getState()
                    ^ QuReg.shiftLeftTruncate(target1)
                    ^ QuReg.shiftLeftTruncate(target2), reg);
    ...
    void quantum_cond_phase(int control, int target, QuantumReg reg) {
        ...
        //return cos(phi) + IMAGINARY * sin(phi);
        double angle = Math.PI / QuReg.shiftLeftTruncate(control - target);
        ComplexModifiable z = new ComplexModifiable(Math.cos(angle), Math.sin(angle));
    ...
    void quantum_toffoli_ft(int control1, int control2, int target, 
        ...
        mask = QuReg.shiftLeftTruncate(target)
            + QuReg.shiftLeftTruncate(target + width)
            + QuReg.shiftLeftTruncate(target + 2 * width);
